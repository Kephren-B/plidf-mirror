...
def meta_description(soup) -> str:
    m = soup.select_one('meta[property="og:description"]') or soup.select_one(
        'meta[name="description"]'
    )
    return clean(m.get("content")) if m is not None else ""


def line_content(soup):
    """(subtitle, [paragraphes]) : terminus + présentation d'une page ligne."""
    h1 = soup.select_one("#top") or soup.select_one("article h1") or soup.select_one("h1")
    subtitle, paras = "", []
    if h1 is None:
        return subtitle, paras
    for el in h1.find_all_next():
        ...
        if el.name == "p":
            ...
            if not t:
                continue
            if el.find("em") is not None and not subtitle:
                subtitle = t
            elif el.get("class") and "section-link" in el.get("class", []):
                continue
            else:
                paras.append(t)
    return subtitle, paras
...
def related_projects(soup):
    """Projets associés (dl.projects-dl) d'une page ligne."""
    out = []
    dl = soup.select_one("dl.projects-dl") or soup.select_one("#projects-section dl")
    if dl is None:
        return out
    for dt in dl.select("dt"):
        ...
        desc = text_of(dd) if dd is not None else ""
        if uid:
            out.append({"uid": uid, "name": text_of(a) or uid, "desc": desc})
    return out
...
def parse_gare(soup, uid, rel):
    ...
    return {
        ...
        "propositions": parse_project_links(soup),
        "description": meta_description(soup), "file": rel,
    }
...
def parse_ligne(soup, uid, rel, kind):
    ...
    subtitle, paras = line_content(soup)
    return {
        "type": kind, "uid": uid, "name": name, "mode": mode or None,
        "subtitle": subtitle or None, "content": paras,
        ...
        "projects": parse_project_links(soup),
        "relatedProjects": related_projects(soup),
        "description": meta_description(soup), "file": rel,
    }
