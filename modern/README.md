...
Les SVG des logos de lignes sont dans `public/logos/`, servis via
`/logos/<fichier>.svg` (voir `src/logos.ts` pour le mapping UID → fichier).

Deux origines coexistent :
- **Icônes officielles « Paris transit icons »** (Wikimedia Commons) pour les
  lignes existantes (métro, RER A–E, transilien, tramways) — licences libres.
- **Logos d'origine legacy** (miroir `../legacy`) pour les lignes/projets
  proposés sans équivalent officiel (Cergyval, tangentielles, RER F/G…).

Compléter/mettre à jour les icônes officielles :
```powershell
powershell -ExecutionPolicy Bypass -File tools/fetch_paris_transit_logos.ps1
```
> Wikimedia limite le débit (HTTP 429) : si le script échoue partiellement,
> relancez-le simplement plus tard — il est idempotent.
